...
<template>
  <main class="order-tracking-page">
    ...
    <section v-else-if="error" class="order-tracking-state order-tracking-error">
      <div class="order-tracking-state-icon">!</div>
      <h1>Order not found</h1>
      <p>{{ error }}</p>
      <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center">
        <RouterLink :to="`/${storeSlug}/track`" class="button button-primary">Look up by phone</RouterLink>
        <RouterLink :to="`/${storeSlug}`" class="button button-secondary">Back to store</RouterLink>
      </div>
    </section>
    ...
    <section v-else-if="order" class="order-tracking-shell">
      <div class="order-tracking-heading">
        <div>
          ...
          <p>Hi {{ order.customer_first_name }}, your order is being updated automatically.</p>
        </div>
