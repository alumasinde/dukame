...
const paymentMessage = computed(() => {
  ...
  if (order.value.payment.status === 'paid') return 'Payment received. Your order has been placed successfully.'
  if (order.value.payment.status === 'processing' && order.value.payment.method.code === 'mpesa') return `M-Pesa prompt sent to ${order.value.customer_phone}. Check your phone and enter your M-Pesa PIN to complete payment.`
  if (order.value.payment.status === 'pending' && order.value.payment.method.code === 'cash') return 'Pay in cash when your order is delivered or collected.'
  if (order.value.payment.status === 'failed') return order.value.payment.failure_reason || 'The selected payment could not be completed.'
...
async function submitOrder() { if (!cart.value?.items.length || submitting.value) return; fieldError.value = ''; error.value = ''; if (!form.first_name.trim() || !form.last_name.trim() || !form.phone.trim()) { fieldError.value = 'Enter your first name, last name and phone number to continue.'; return } if (!selectedPaymentMethod.value) { fieldError.value = 'Choose a payment method to continue.'; return } submitting.value = true; try { order.value = (await checkoutCart(storeSlug, { first_name: form.first_name.trim(), last_name: form.last_name.trim(), phone: form.phone.trim(), email: form.email.trim() || undefined, notes: form.notes.trim() || undefined, payment_method_public_id: selectedPaymentMethod.value })).data; storeName.value = order.value.store_name || storeName.value; cart.value = null; cartState.clear(storeSlug) } catch (err: any) { error.value = apiError(err, 'We could not place your order. Please try again.'); await reconcileCart() } finally { submitting.value = false } }
...
<template>
  <main class="storefront-page">
    <header class="storefront-header">
      <RouterLink :to="`/${storeSlug}`" class="storefront-brand storefront-brand-link"><span class="storefront-mark">{{ storeName ? storeName.charAt(0).toUpperCase() : 'S' }}</span><div><strong>{{ storeName || 'Your store' }}</strong><small>Powered by DukaMe</small></div></RouterLink>
    ...
    <section class="storefront-cart-page">
      ...
      <div v-else-if="order" class="storefront-order-success">
        ...
        <div class="storefront-success-summary"><div><span>Payment</span><strong>{{ order.payment?.status === 'paid' ? 'Paid' : order.payment?.status === 'processing' ? 'Payment pending' : order.payment?.status === 'failed' ? 'Payment failed' : 'Payment pending' }}</strong></div><div><span>Total</span><strong>{{ money(order.total_minor, order.currency) }}</strong></div><div><span>Items</span><strong>{{ order.items.reduce((sum, item) => sum + item.quantity, 0) }}</strong></div></div>
        <div v-if="paymentMessage" class="storefront-payment-result" :class="`is-${order.payment?.status || 'pending'}`"><strong>{{ order.payment?.method.name }}</strong><p>{{ paymentMessage }}</p><button v-if="order.payment?.status === 'processing'" type="button" @click="refreshPaymentStatus">Check payment status</button></div>
        ...
        <p class="storefront-success-notice">Your tracking page will show payment confirmation and order progress.</p>
      </div>
      <template v-else-if="cart">
        ...
        <div v-if="cart.items.length" class="storefront-cart-layout">
          ...
          <aside class="storefront-checkout-card"><div class="storefront-checkout-title"><strong>Checkout</strong><span>{{ itemCountLabel }}</span></div><div class="storefront-order-total"><span>Subtotal</span><strong>{{ money(cart.subtotal_minor, cart.currency) }}</strong></div><div class="storefront-form-grid"><label><span>First name</span><input v-model="form.first_name" autocomplete="given-name" /></label><label><span>Last name</span><input v-model="form.last_name" autocomplete="family-name" /></label><label><span>Phone</span><input v-model="form.phone" type="tel" autocomplete="tel" placeholder="07xx xxx xxx" /></label><label><span>Email <em>Optional</em></span><input v-model="form.email" type="email" autocomplete="email" /></label><label class="storefront-form-full"><span>Order note <em>Optional</em></span><textarea v-model="form.notes" rows="3" placeholder="Anything the store should know?"></textarea></label></div>
            <div class="storefront-payment-section"><div class="storefront-payment-heading"><strong>Payment method</strong><span>Choose how you want to pay</span></div><div v-if="loadingPaymentMethods" class="storefront-payment-loading">Loading payment options…</div><div v-else-if="paymentMethods.length" class="storefront-payment-options"><label v-for="method in paymentMethods" :key="method.public_id" class="storefront-payment-option" :class="{ selected: selectedPaymentMethod === method.public_id }"><input v-model="selectedPaymentMethod" type="radio" name="payment_method" :value="method.public_id" /><span class="storefront-payment-radio" aria-hidden="true"></span><span class="storefront-payment-copy"><strong>{{ method.name }}</strong><small>{{ method.code === 'mpesa' ? 'Pay securely with M-Pesa' : method.code === 'cash' ? 'Pay when your order is delivered or collected' : method.instructions }}</small></span><span v-if="method.code === 'mpesa'" class="storefront-payment-tag">Recommended</span></label></div><p v-else class="storefront-form-error">No payment methods are currently available.</p></div>
            <p v-if="selectedMethod?.instructions" class="storefront-checkout-note">{{ selectedMethod.instructions }}</p><p v-if="fieldError" class="storefront-form-error">{{ fieldError }}</p><button class="button button-primary button-lg storefront-add" type="button" :disabled="submitting || loadingPaymentMethods || !paymentMethods.length" @click="submitOrder">{{ submitting ? 'Placing order…' : 'Place order' }}</button><p class="storefront-payment-legal">Your payment is handled securely. Never share your M-Pesa PIN with anyone.</p>
          </aside>
        </div>
