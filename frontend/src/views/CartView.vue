...
const checkoutStep = ref<'cart' | 'shipping' | 'payment' | 'review'>('cart')
...
const form = reactive({ first_name: '', last_name: '', phone: '', email: '', delivery_address: '', delivery_landmark: '', delivery_notes: '', delivery_option: 'standard', notes: '' })
...
function validateShipping() {
  ...
  if (!form.first_name.trim() || !form.last_name.trim()) {
    fieldError.value = 'Enter your first name and last name to continue.'
    return false
  }
  ...
  if (form.delivery_address.trim().length < 5) {
    fieldError.value = 'Enter a delivery address so the rider knows where to go.'
    return false
  }
  return true
}
function goToShipping() { if (!cart.value?.items.length) return; error.value = ''; checkoutStep.value = 'shipping' }
function goToPayment() { fieldError.value = ''; error.value = ''; if (validateShipping()) checkoutStep.value = 'payment' }
function goToReview() {
  fieldError.value = ''
  error.value = ''
  if (!validateShipping()) return
  if (!selectedPaymentMethod.value) {
    fieldError.value = 'Choose a payment method to continue.'
    return
  }
...
async function submitOrder() {
  if (!cart.value?.items.length || submitting.value || !selectedPaymentMethod.value) return
  fieldError.value = ''
  error.value = ''
  if (!validateShipping()) {
  ...
  try {
    order.value = (await checkoutCart(storeSlug, {
      ...
      delivery_landmark: form.delivery_landmark.trim() || undefined,
      delivery_notes: form.delivery_notes.trim() || undefined,
      delivery_option: form.delivery_option,
      notes: form.notes.trim() || undefined,
      payment_method_public_id: selectedPaymentMethod.value,
    })).data
    storeName.value = order.value.store_name || storeName.value
    cart.value = null
    ...
    <section class="storefront-cart-page">
      ...
      <template v-else-if="cart">
        ...
        <div v-if="cart.items.length" class="storefront-checkout-progress" aria-label="Checkout progress">
          <div class="storefront-checkout-step" :class="{ active: checkoutStep === 'cart', complete: checkoutStep !== 'cart' }"><span>1</span>Cart</div>
          <div class="storefront-checkout-step" :class="{ active: checkoutStep === 'shipping', complete: checkoutStep === 'payment' || checkoutStep === 'review' }"><span>2</span>Details</div>
          <div class="storefront-checkout-step" :class="{ active: checkoutStep === 'payment', complete: checkoutStep === 'review' }"><span>3</span>Pay</div>
          <div class="storefront-checkout-step" :class="{ active: checkoutStep === 'review' }"><span>4</span>Review</div>
        </div>
        ...
        <section v-else-if="checkoutStep === 'shipping'" class="storefront-checkout-panel"><h2>Enter your details</h2><p>Use the phone number that receives M-Pesa prompts and order SMS.</p><div class="storefront-form-grid"><label><span>First name</span><input v-model="form.first_name" autocomplete="given-name" /></label><label><span>Last name</span><input v-model="form.last_name" autocomplete="family-name" /></label><label class="storefront-form-full"><span>Phone (M-Pesa)</span><input v-model="form.phone" type="tel" inputmode="tel" autocomplete="tel" placeholder="07XX XXX XXX" /><p class="storefront-phone-hint" :class="{ ok: phoneValid && form.phone.trim() }">{{ form.phone.trim() ? (phoneValid ? `Looks good · ${formatKenyaPhoneDisplay(form.phone)}` : 'Use a Kenyan mobile number such as 07XX XXX XXX') : 'This number receives the M-Pesa PIN prompt and order updates.' }}</p></label><label><span>Email <em>Optional</em></span><input v-model="form.email" type="email" autocomplete="email" /></label><label class="storefront-form-full"><span>Delivery option</span><select v-model="form.delivery_option"><option value="standard">Standard delivery</option><option value="express">Express delivery</option><option value="pickup">Pickup from store</option></select></label><label class="storefront-form-full"><span>Delivery address</span><textarea v-model="form.delivery_address" rows="2" autocomplete="street-address" placeholder="Street, building, house/office number, area"></textarea></label><label class="storefront-form-full"><span>Nearby landmark <em>Optional</em></span><input v-model="form.delivery_landmark" placeholder="e.g. Next to Naivas supermarket" /></label><label class="storefront-form-full"><span>Delivery notes <em>Optional</em></span><textarea v-model="form.delivery_notes" rows="2" placeholder="Gate code, preferred time, how to find you"></textarea></label><label class="storefront-form-full"><span>Order note <em>Optional</em></span><textarea v-model="form.notes" rows="3" placeholder="Anything else the store should know"></textarea></label></div><p v-if="fieldError" class="storefront-form-error">{{ fieldError }}</p><div class="storefront-checkout-actions"><button class="button button-secondary" type="button" @click="checkoutStep = 'cart'">Back to cart</button><button class="button button-primary button-lg" type="button" @click="goToPayment">Continue to payment</button></div></section>
        ...
        <section v-else class="storefront-checkout-panel">
          <h2>Review & place order</h2>
          <p>Confirm your items, contact details and payment method.</p>
          <div class="storefront-review-block">
            <h3>Items</h3>
            <div class="storefront-review-items">
              <div v-for="item in cart.items" :key="item.public_id" class="storefront-review-item">
                <div><strong>{{ item.product_name }}</strong><small v-if="item.variant_label">{{ item.variant_label }}</small><small>{{ item.quantity }} × {{ money(item.unit_price_minor, item.currency) }}</small></div>
          ...
          <div class="storefront-review-block">
            <h3>Delivery</h3>
            <div class="storefront-review-row"><span>Option</span><strong>{{ form.delivery_option === 'pickup' ? 'Pickup from store' : form.delivery_option === 'express' ? 'Express delivery' : 'Standard delivery' }}</strong></div>
            <div class="storefront-review-row"><span>Address</span><strong>{{ form.delivery_address }}</strong></div>
            <div v-if="form.delivery_landmark.trim()" class="storefront-review-row"><span>Landmark</span><strong>{{ form.delivery_landmark }}</strong></div>
            <div v-if="form.delivery_notes.trim()" class="storefront-review-row"><span>Delivery notes</span><strong>{{ form.delivery_notes }}</strong></div>
          </div>
          <div class="storefront-review-block">
            <h3>Payment</h3>
