...
function recalculate(next: Cart): Cart {
  next.item_count = next.items.reduce((sum, item) => sum + item.quantity, 0)
  next.subtotal_minor = next.items.reduce((sum, item) => sum + item.line_total_minor, 0)
  return next
}

async function reconcileCart() {
  try {
    const fresh = await cartState.load(storeSlug, true)
    cart.value = fresh
  } catch (err: any) {
    if (!cart.value) error.value = apiError(err, 'We could not load your cart.')
  } finally {
    loading.value = false
  }
}
...
async function submitOrder() {
  ...
  if (!form.first_name.trim() || !form.last_name.trim() || !form.phone.trim()) {
    fieldError.value = 'Enter your first name, last name and phone number to continue.'
    return
  }
  ...
  <main class="storefront-page">
    ...
    <section class="storefront-cart-page">
      ...
      <div v-else-if="order" class="storefront-order-success">
        <div class="storefront-success-icon">✓</div><span class="storefront-eyebrow">Order received</span><h1>Thank you, {{ order.customer_first_name }}.</h1><p>Your order <strong>#{{ order.order_number }}</strong> has been received. The store will contact you on {{ order.customer_phone }}.</p>
          ...
          <section class="storefront-cart-items" aria-label="Cart items">
            <article v-for="item in cart.items" :key="item.public_id" class="storefront-cart-item" :class="{ 'is-busy': busyItems[item.public_id] }">
              ...
              <div class="storefront-cart-item-copy">
                <div class="storefront-cart-item-title"><RouterLink :to="`/${storeSlug}/products/${item.product_slug}`"><strong>{{ item.product_name }}</strong></RouterLink><button type="button" class="storefront-remove" :disabled="busyItems[item.public_id]" @click="remove(item.public_id)">Remove</button></div>
                <small v-if="item.variant_label">{{ item.variant_label }}</small><small v-if="item.sku" class="storefront-item-sku">SKU: {{ item.sku }}</small>
                <div class="storefront-cart-item-bottom"><div class="storefront-quantity" :class="{ disabled: busyItems[item.public_id] }"><button type="button" aria-label="Decrease quantity" :disabled="item.quantity <= 1 || busyItems[item.public_id]" @click="changeQuantity(item.public_id, item.quantity - 1)">−</button><span aria-live="polite">{{ item.quantity }}</span><button type="button" aria-label="Increase quantity" :disabled="busyItems[item.public_id]" @click="changeQuantity(item.public_id, item.quantity + 1)">+</button></div><div class="storefront-cart-item-price"><span>{{ money(item.unit_price_minor, item.currency) }} each</span><strong>{{ money(item.line_total_minor, item.currency) }}</strong></div></div>
              </div>
            </article>
          </section>

          <aside class="storefront-checkout-card">
            <div class="storefront-checkout-title"><strong>Checkout</strong><span>{{ itemCountLabel }}</span></div><div class="storefront-order-total"><span>Subtotal</span><strong>{{ money(cart.subtotal_minor, cart.currency) }}</strong></div><p class="storefront-checkout-note">Delivery and payment details will be confirmed by the store.</p>
            <div class="storefront-form-grid"><label><span>First name</span><input v-model="form.first_name" autocomplete="given-name" /></label><label><span>Last name</span><input v-model="form.last_name" autocomplete="family-name" /></label><label><span>Phone</span><input v-model="form.phone" type="tel" autocomplete="tel" placeholder="07xx xxx xxx" /></label><label><span>Email <em>Optional</em></span><input v-model="form.email" type="email" autocomplete="email" /></label><label class="storefront-form-full"><span>Order note <em>Optional</em></span><textarea v-model="form.notes" rows="3" placeholder="Anything the store should know?"></textarea></label></div>
            <p v-if="fieldError" class="storefront-form-error">{{ fieldError }}</p><button class="button button-primary button-lg storefront-add" type="button" :disabled="submitting" @click="submitOrder">{{ submitting ? 'Placing order…' : 'Place order' }}</button>
          </aside>
        </div>
