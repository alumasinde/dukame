    ...
    <template v-else-if="store">
      ...
      <section class="storefront-content">
        <div class="storefront-shop-heading">
          ...
          <div class="storefront-search-wrap">
            ...
            <div v-if="showRecent && recentSearches.length" class="storefront-recent" role="listbox">
              <div class="storefront-recent-head"><span>Recent</span><button type="button" @mousedown.prevent="clearRecentSearches(storeSlug); recentSearches = []">Clear</button></div>
        ...
        <div class="storefront-toolbar">
          <button type="button" class="storefront-filter-chip" :class="{ active: inStockOnly }" @click="inStockOnly = !inStockOnly">{{ inStockOnly ? 'In stock only ✓' : 'In stock only' }}</button>
        ...
        <div id="storefront-products" class="storefront-products-section">
          <div v-if="pagedProducts.length" class="storefront-grid">
            <article v-for="product in pagedProducts" :key="product.public_id" class="storefront-product" :class="{ 'storefront-product-unavailable': !isAvailable(product) }">
              <div class="storefront-product-image">
                ...
                <button type="button" class="storefront-wishlist" :class="{ 'is-favorite': isFavorite(product.public_id) }" :aria-label="isFavorite(product.public_id) ? `Remove ${product.name} from favourites` : `Add ${product.name} to favourites`" :aria-pressed="isFavorite(product.public_id)" @click="toggleFavorite(product.public_id)">♥</button>
              </div>
              <RouterLink :to="`/${store.slug}/products/${product.slug}`" class="storefront-product-link">
              ...
              <div class="storefront-product-action">
                <template v-if="!hasVariants(product) && isAvailable(product)">
                  <div v-if="cartItemFor(product)" class="storefront-product-quantity" :class="{ busy: quantityBusy === cartItemFor(product)?.public_id }">
                    ...
                    <button type="button" aria-label="Increase quantity" :disabled="quantityBusy === cartItemFor(product)?.public_id" @click="changeProductQuantity(product, 1)">+</button>
                  </div>
          ...
          <div v-else-if="searchQuery || selectedCategory || inStockOnly" class="storefront-empty storefront-empty-search">
            ...
            <div class="storefront-empty-actions"><button class="button button-secondary" type="button" @click="clearAllFilters">Clear all filters</button></div>
          </div>
          <div v-else class="storefront-empty">
            <div class="storefront-empty-icon">⌂</div>
            <h2>No products yet</h2>
            <p>This store is getting ready. Check back soon.</p>
          </div>
      ...
      <Teleport to="body">
        ...
        <aside v-if="showCartDrawer" class="storefront-cart-drawer" role="dialog" aria-modal="true" aria-label="Shopping cart">
          <div class="storefront-cart-drawer-head"><div><strong>Your cart</strong></div><button type="button" class="storefront-drawer-close" aria-label="Close cart" @click="closeDrawer">×</button></div>
            ...
            <article v-for="item in cartState.cart.value?.items || []" :key="item.public_id" class="storefront-mini-cart-item">
              ...
              <div class="storefront-mini-cart-copy">
                ...
                <div class="storefront-mini-cart-actions">
                  <div class="storefront-quantity"><button type="button" @click="changeDrawerQuantity(item.public_id, item.quantity - 1)">−</button><span>{{ item.quantity }}</span><button type="button" @click="changeDrawerQuantity(item.public_id, item.quantity + 1)">+</button></div>
                  <button type="button" class="storefront-mini-remove" @click="changeDrawerQuantity(item.public_id, 0)">Remove</button>
                </div>
              </div>
            </article>
            <div v-if="!cartState.cart.value?.items.length" class="storefront-empty" style="min-height:240px"><h2>Your cart is empty</h2></div>
          </div>
