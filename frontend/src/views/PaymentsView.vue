...
async function load() {
  ...
  try {
    const [methodsResponse, paymentsResponse] = await Promise.all([getPaymentMethodsForTenant(tenantId.value), getPayments(tenantId.value, { limit: 50 })])
...
async function saveMpesa() {
  ...
  try {
    ...
    callbackUrl.value = response.data.callback_url || ''; callbackToken.value = response.data.callback_token || ''
...
async function settleCash(payment: PaymentListItem) {
  if (!tenantId.value || saving.value) return
  saving.value = true; error.value = ''; success.value = ''
  try {
    await markCashPaymentPaid(tenantId.value, payment.public_id)
    success.value = `Payment for ${payment.order_number} marked as paid.`
    await load()
  } catch (err: any) { error.value = apiError(err, 'We could not settle the cash payment.') }
  finally { saving.value = false }
}
...
async function retryMpesa(payment: PaymentListItem) {
  ...
  saving.value = true; error.value = ''; success.value = ''
  try {
    await retryPayment(tenantId.value, payment.public_id)
    success.value = 'A new M-Pesa payment prompt has been sent.'
    await load()
...
<template>
  <section class="page payments-page">
    <div class="page-header"><div><span class="page-eyebrow">Commerce</span><h1>Payments</h1><p>Manage how customers pay and review payment activity.</p></div></div>
    ...
    <template v-else>
      <section class="payments-methods-card">
        <div class="payments-card-head"><div><h2>Payment methods</h2><p>Disabled methods disappear from checkout. Existing payments are not affected.</p></div><button v-if="!hasMpesa" class="button button-primary" type="button" @click="showMpesaForm = !showMpesaForm">{{ showMpesaForm ? 'Cancel' : 'Add M-Pesa' }}</button></div>
        <div class="payments-method-list">
          <article v-for="method in methods" :key="method.public_id" class="payments-method-row"><div class="payments-method-icon">{{ method.code === 'mpesa' ? 'M' : 'C' }}</div><div class="payments-method-copy"><strong>{{ method.name }}</strong><span>{{ method.code === 'mpesa' ? 'M-Pesa STK Push' : method.instructions }}</span></div><span class="payments-enabled">{{ method.is_enabled ? 'Enabled' : 'Disabled' }}</span><button class="button button-secondary" type="button" :disabled="saving" @click="toggleMethod(method)">{{ method.is_enabled ? 'Disable' : 'Enable' }}</button></article>
        </div>
      </section>

      <section v-if="showMpesaForm && !hasMpesa" class="payments-config-card">
        <div class="payments-card-head"><div><h2>Connect M-Pesa</h2><p>Credentials are encrypted before they are stored and are never returned to the browser.</p></div></div>
        <div class="payments-form-grid"><label><span>Display name</span><input v-model="form.name" /></label><label><span>Environment</span><select v-model="form.environment"><option value="sandbox">Sandbox</option><option value="production">Production</option></select></label><label><span>Consumer key</span><input v-model="form.consumer_key" autocomplete="off" /></label><label><span>Consumer secret</span><input v-model="form.consumer_secret" type="password" autocomplete="new-password" /></label><label><span>Business shortcode</span><input v-model="form.shortcode" inputmode="numeric" /></label><label><span>Passkey</span><input v-model="form.passkey" type="password" autocomplete="new-password" /></label><label><span>Transaction type</span><select v-model="form.transaction_type"><option value="CustomerPayBillOnline">CustomerPayBillOnline</option><option value="CustomerBuyGoodsOnline">CustomerBuyGoodsOnline</option></select></label><label><span>Account reference</span><input v-model="form.account_reference" maxlength="12" /></label><label class="payments-form-full"><span>Checkout instructions</span><textarea v-model="form.instructions" rows="2"></textarea></label></div>
        <div class="payments-config-actions"><button class="button button-primary" type="button" :disabled="saving" @click="saveMpesa">{{ saving ? 'Connecting…' : 'Connect M-Pesa' }}</button></div>
      </section>
