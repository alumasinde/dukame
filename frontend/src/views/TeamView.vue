...
const auth = useAuthStore(); const members = ref<any[]>([]); const roles = ref<any[]>([]); const permissions = ref<any[]>([]); const loading = ref(true); const error = ref(''); const selected = ref('')
...
async function updateRole(member: any) { if (!tenantId.value || !selected.value) return; const role = roles.value.find((r) => r.public_id === selected.value); if (!role) return; try { await api.put(`/tenants/${tenantId.value}/members/${member.user_public_id}/role`, { role_public_id: role.public_id }); await load() } catch (err: any) { error.value = err?.response?.data?.detail || 'Role update failed.' } finally { selected.value = '' } }
onMounted(load)
</script>
