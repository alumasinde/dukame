...
<template>
  <div class="page-stack">
    ...
    <section v-if="!store && !storeLoading" class="panel"><div class="panel-heading"><div><h3>Set up your store</h3><p>Your shop is ready. Complete these details to start adding products.</p></div></div><form class="form-grid" @submit.prevent="setupStore"><label>Store name<input v-model="storeForm.name" required /></label><label>Store link<input v-model="storeForm.slug" @blur="storeForm.slug = slugify(storeForm.slug)" required /></label><label>Currency<input v-model="storeForm.currency" maxlength="3" placeholder="KES" required /></label><label>Status<select v-model="storeForm.status"><option value="active">Active</option><option value="inactive">Inactive</option></select></label><label class="form-span-2">Description<textarea v-model="storeForm.description" rows="3" /></label><div><button class="button button-primary" :disabled="saving">{{ saving ? 'Creating…' : 'Save shop' }}</button></div></form></section>

    <template v-else-if="store">
      <section class="catalogue-toolbar panel"><div class="search-box"><i class="fa-solid fa-magnifying-glass" aria-hidden="true"></i><input v-model="search" placeholder="Search products or SKU" /></div><select v-model="statusFilter"><option value="all">All statuses</option><option v-for="status in [...new Set(products.map(product => product.status))]" :key="status" :value="status">{{ status }}</option></select><span class="result-count">{{ filteredProducts.length }} product{{ filteredProducts.length === 1 ? '' : 's' }}</span></section>
      <section class="panel"><div v-if="loading" class="mini-empty">Loading products…</div><div v-else-if="!filteredProducts.length" class="empty-state compact"><div class="empty-illustration"><i class="fa-solid fa-box" aria-hidden="true"></i></div><h3>{{ products.length ? 'No products found' : 'Add your first product' }}</h3><p>{{ products.length ? 'Try a different name, SKU or status.' : 'Add the product name, price, images and stock details customers need.' }}</p><button v-if="!products.length" class="button button-primary" @click="openEditor()">Add product</button></div><div v-else class="table-wrap"><table><thead><tr><th>Product</th><th>Category</th><th>Price</th><th>Inventory</th><th>Status</th><th></th></tr></thead><tbody><tr v-for="product in filteredProducts" :key="product.public_id"><td><strong>{{ product.name }}</strong><small>{{ product.sku || product.slug }}</small></td><td>{{ categoryName(product.category_public_id) }}</td><td>{{ product.currency }} {{ (product.price_minor / 100).toFixed(2) }}</td><td>{{ product.inventory_tracking ? product.inventory_quantity : 'Not tracked' }}</td><td><span class="status-pill">{{ product.status }}</span></td><td class="actions"><button class="button button-small" @click="openEditor(product.public_id)">Edit</button><button class="button button-small button-danger" :disabled="deleting === product.public_id" @click="deleteProduct(product)">{{ deleting === product.public_id ? 'Deleting…' : 'Delete' }}</button></td></tr></tbody></table></div></section>
    </template>
  </div>
</template>
