...
ROLE_PERMISSIONS = {
    "admin": {p[0] for p in PERMISSIONS if p[0] not in {"subscription.manage"}},
...
def upgrade() -> None:
    ...
    op.execute(
        sa.text(
            "INSERT INTO tenant_roles (public_id, tenant_id, name, slug, is_system) "
            "SELECT REPLACE(UUID(), '-', ''), id, 'Owner', 'owner', 1 FROM tenants"
        )
    )
    op.execute(
        sa.text(
            "INSERT INTO tenant_roles (public_id, tenant_id, name, slug, is_system) "
            "SELECT REPLACE(UUID(), '-', ''), id, 'Administrator', 'admin', 1 FROM tenants"
        )
    )
    op.execute(
        sa.text(
            "INSERT INTO tenant_roles (public_id, tenant_id, name, slug, is_system) "
            "SELECT REPLACE(UUID(), '-', ''), id, 'Manager', 'manager', 1 FROM tenants"
        )
    )
    op.execute(
        sa.text(
            "INSERT INTO tenant_roles (public_id, tenant_id, name, slug, is_system) "
            "SELECT REPLACE(UUID(), '-', ''), id, 'Staff', 'staff', 1 FROM tenants"
        )
    )
    op.execute(
        sa.text(
            "UPDATE tenant_users tu JOIN tenant_roles tr ON tr.tenant_id = tu.tenant_id AND tr.slug = "
            "CASE WHEN tu.role IN ('owner','admin','manager','staff') THEN tu.role ELSE 'owner' END "
            "SET tu.role_id = tr.id"
        )
    )
    ...
    for slug, keys in ROLE_PERMISSIONS.items():
        ...
        op.execute(
            sa.text(
                ...
                "SELECT tr.id, p.id FROM tenant_roles tr JOIN permissions p "
                f"ON p.key IN ({placeholders}) WHERE tr.slug = :slug"
            ),
            {**params, "slug": slug},
        )
    ...
    op.execute(
        sa.text(
            "INSERT INTO tenant_role_permissions (role_id, permission_id) "
            "SELECT tr.id, p.id FROM tenant_roles tr CROSS JOIN permissions p WHERE tr.slug = 'owner'"
        )
    )
