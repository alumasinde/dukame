...
down_revision = "0014_order_tracking_hash_compatibility"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("provider_response_message", sa.String(length=1000), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    ...
    for store in stores:
        bind.execute(
            sa.insert(payment_methods).values(
                ...
                is_enabled=True,
                sort_order=10,
                instructions="Pay the store in cash when your order is delivered or collected.",
            )
        )
    ...
    role_permission = sa.table("tenant_role_permissions", sa.column("role_id", sa.BigInteger), sa.column("permission_id", sa.BigInteger))
