...
class StoreService:
    ...
    async def create(self, user: User, tenant_public_id: str, payload: StoreCreate):
        ...
        try:
            store = await self.repository.create(public_id=uuid.uuid4().hex, tenant_id=tenant.id, **payload.model_dump())
            await self.db.commit()
        except IntegrityError:
            await self.db.rollback()
            raise HTTPException(status_code=409, detail="Store could not be created with the supplied values") from None
    ...
    async def update(self, user: User, tenant_public_id: str, payload: StoreUpdate):
        ...
        if "slug" in values and values["slug"] != store.slug and await self.repository.get_by_slug(values["slug"]):
            raise HTTPException(status_code=409, detail="Store slug already exists")
        ...
        try:
            ...
            await self.db.rollback()
            raise HTTPException(status_code=409, detail="Store could not be updated with the supplied values") from None
        await self.db.refresh(store)
        return store
