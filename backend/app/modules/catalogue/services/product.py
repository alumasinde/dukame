...
from app.modules.subscriptions.services.gating import enforce_product_capacity
...
class ProductService:
    ...
    async def create(self, user: User, tenant_public_id: str, payload: ProductCreate):
        ...
        await enforce_product_capacity(self.db, store.tenant_id)
        if payload.currency != store.currency:
            raise HTTPException(
                status_code=422,
                detail=f"Product currency must match the store currency ({store.currency})",
            )
        category_id = None
        ...
        try:
            product = await self.repository.create(
                ...
                **payload.model_dump(exclude={"category_public_id"}),
            )
            ...
            raise HTTPException(
                status_code=409,
                detail="Product could not be created with the supplied values",
            ) from None
        return await self.repository.get(store.id, product.public_id)

    async def update(
        self, user: User, tenant_public_id: str, public_id: str, payload: ProductUpdate
    ):
        store = await resolve_store(self.db, user, tenant_public_id, "catalogue.manage")
        product = await self.repository.get(store.id, public_id)
        if product is None:
        ...
        if "inventory_quantity" in values:
            raise HTTPException(
                status_code=409,
                detail="Inventory quantity must be changed through the inventory adjustment API",
            )
        ...
        if (
            "slug" in values
            and values["slug"] != product.slug
            and await self.repository.get_by_slug(store.id, values["slug"])
        ):
            raise HTTPException(status_code=409, detail="Product slug already exists")
        if (
            "sku" in values
            and values["sku"]
            and values["sku"] != product.sku
            and await self.repository.get_by_sku(store.id, values["sku"])
        ):
            raise HTTPException(status_code=409, detail="Product SKU already exists")
        ...
        if (
            "price_minor" in values
            and "compare_at_price_minor" not in values
            and product.compare_at_price_minor is not None
            and product.compare_at_price_minor < values["price_minor"]
        ):
            raise HTTPException(
                status_code=422,
                detail="compare_at_price_minor must be greater than or equal to price_minor",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=409,
                detail="Product could not be updated with the supplied values",
            ) from None
