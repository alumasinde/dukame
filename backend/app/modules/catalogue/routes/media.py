...
from app.modules.catalogue.schemas.media import (
    ProductMediaCreate,
...
router = APIRouter(
    prefix="/tenants/{tenant_public_id}/products/{product_public_id}/media",
    tags=["catalogue"],
)
...
async def list_media(
    ...
    return [
        to_response(item)
        for item in await ProductMediaService(db).list(
            user, tenant_public_id, product_public_id
        )
    ]
...
async def create_media(
    ...
    media = await ProductMediaService(db).create(
        user, tenant_public_id, product_public_id, payload
    )
...
async def update_media(
    ...
    media = await ProductMediaService(db).update(
        user,
        tenant_public_id,
        product_public_id,
        media_public_id,
...
async def delete_media(
    ...
    await ProductMediaService(db).delete(
        user, tenant_public_id, product_public_id, media_public_id
    )
