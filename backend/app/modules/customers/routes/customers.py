...
def to_response(result) -> CustomerResponse:
...
async def create_customer(tenant_public_id: str, payload: CustomerCreate, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> CustomerResponse:
...
async def get_customer(tenant_public_id: str, customer_public_id: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> CustomerResponse:
...
async def update_customer(tenant_public_id: str, customer_public_id: str, payload: CustomerUpdate, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> CustomerResponse:
