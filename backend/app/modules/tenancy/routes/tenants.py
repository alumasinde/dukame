...
async def list_tenants(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> TenantListResponse:
    ...
    return TenantListResponse(items=[TenantResponse(public_id=t.public_id, name=t.name, slug=t.slug, status=t.status, role=membership.role, business_type_public_id=t.business_type.public_id if t.business_type else None, business_type_name=t.business_type.name if t.business_type else None) for t, membership in rows])
...
async def create_shop(payload: CreateTenantRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> TenantResponse:
    ...
    tenant, _ = await create_tenant(db, user, payload.name, payload.slug, business_type.id)
    return TenantResponse(public_id=tenant.public_id, name=tenant.name, slug=tenant.slug, status=tenant.status, role="owner", business_type_public_id=business_type.public_id, business_type_name=business_type.name)
