...
async def list_storefront_payment_methods(store_slug: str, db: AsyncSession = Depends(get_db)) -> list[PaymentMethodResponse]:
    ...
    return [PaymentMethodResponse(public_id=item.public_id, code=item.code, name=item.name, instructions=item.instructions) for item in methods]
...
async def create_payment_method(tenant_public_id: str, payload: PaymentMethodCreate, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> PaymentMethodResponse:
    ...
    return PaymentMethodResponse(public_id=method.public_id, code=method.code, name=method.name, instructions=method.instructions, callback_url=callback_url, callback_token=callback_token)
