...
from app.modules.auth.services.tokens import (
...
async def _send_auth_email(to: str, subject: str, body: str) -> None:
    try:
        ...
        logger.error("Failed to send auth email", extra={"recipient": to, "error": str(exc)})
...
async def register(
    ...
    user = await register_user(
        db, payload.email, payload.password, payload.first_name, payload.last_name, payload.phone
    )
...
async def resend_verification(
    ...
    return MessageResponse(
        message="If the account exists and needs verification, a verification message has been sent"
    )
...
async def login(
    ...
    user, session, refresh = await authenticate(
        db,
        payload.email,
        payload.password,
...
async def refresh(
    ...
    user, session, refresh_token = await rotate_refresh_token(
        db,
