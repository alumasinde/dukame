...
def _validate_image_header(content_type: str, header: bytes) -> str:
    if content_type == "image/webp":
        if len(header) < 12 or header[:4] != b"RIFF" or header[8:12] != b"WEBP":
            raise HTTPException(status_code=415, detail="The uploaded file is not a valid WebP image")
        return "webp"
...
async def save_product_image(
    ...
    directory = root / "products" / _safe_segment(tenant_public_id) / _safe_segment(product_public_id)
    ...
    return f"/uploads/products/{_safe_segment(tenant_public_id)}/{_safe_segment(product_public_id)}/{filename}"
