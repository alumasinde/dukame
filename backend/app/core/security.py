...
async def get_current_user(credentials: HTTPAuthorizationCredentials | None = Depends(bearer_scheme), db: AsyncSession = Depends(get_db)) -> User:
    ...
    if not isinstance(public_id, str) or not isinstance(session_id, str):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid access token")
    result = await db.execute(select(User).join(AuthSession).where(User.public_id == public_id, AuthSession.public_id == session_id, AuthSession.revoked_at.is_(None), AuthSession.expires_at > utc_now(), User.is_active.is_(True)))
...
def make_session(user: User, user_agent: str | None, ip_address: str | None) -> tuple[AuthSession, str]:
    ...
    session = AuthSession(public_id=uuid.uuid4().hex, user_id=user.id, refresh_token_hash=token_hash(refresh), expires_at=utc_now() + timedelta(seconds=settings.refresh_token_ttl_seconds), user_agent=user_agent[:512] if user_agent else None, ip_address=ip_address)
...
async def revoke_all_user_sessions(db: AsyncSession, user_id: int) -> None:
    await db.execute(update(AuthSession).where(AuthSession.user_id == user_id, AuthSession.revoked_at.is_(None)).values(revoked_at=utc_now()))
